import { W as WebPlugin, E as Encoding } from "./index-1fd16f08.js";
...
class FilesystemWeb extends WebPlugin {
   ...
   */
  async stat(options) {
    const entry = this.getFileHandle(options.path, options);
    return {
      type: entry.type,
      size: entry.size,
      ctime: entry.ctime,
  ...
  async getDirectoryHandle(path, options = {}) {
    ...
    console.log("getDirectoryHandle", this._dirHandle);
    if (!path)
      return handle;
